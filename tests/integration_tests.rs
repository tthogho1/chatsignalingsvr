...
use websocket_chat_signaling_server::{config::ServerConfig, WebSocketServer};
...
async fn test_client_connection_and_disconnection() {
    ...
    assert!(
        close_result.is_ok(),
        "Client should be able to disconnect gracefully"
    );
...
async fn test_broadcast_message_flow() {
    ...
    assert!(
        send_result.is_ok(),
        "Should be able to send broadcast message"
    );
...
async fn test_webrtc_signaling_flow() {
    ...
    assert!(
        send_result.is_ok(),
        "Should be able to send WebRTC signaling message"
    );
...
async fn test_generic_message_flow() {
    ...
    let json_str =
        serde_json::to_string(&generic_message).expect("Should serialize generic message");
    let send_result = write1.send(WsMessage::Text(json_str)).await;
    assert!(
        send_result.is_ok(),
        "Should be able to send generic message"
    );
...
async fn test_multiple_concurrent_connections() {
    ...
    for i in 0..num_clients {
        let handle = tokio::spawn(async move {
            ...
            assert!(
                send_result.is_ok(),
                "Client {} should send message successfully",
                i
            );
    ...
    for handle in handles {
        handle
            .await
            .expect("Client task should complete successfully");
    }
}
...
async fn test_invalid_message_handling() {
    ...
    assert!(
        send_result.is_ok(),
        "Should be able to send invalid message"
    );
...
async fn test_connection_limit() {
    ...
    let (mut write1, _read1) = connect_client(8082)
        .await
        .expect("First client should connect");
    let (mut write2, _read2) = connect_client(8082)
        .await
...
async fn test_message_serialization_compatibility() {
    ...
    for (i, original_message) in test_cases.into_iter().enumerate() {
        ...
        let deserialized: Message =
            serde_json::from_str(&serialized).expect(&format!("Should deserialize message {}", i));

        // Verify fields match
        assert_eq!(
            original_message.id, deserialized.id,
            "Message {} ID should match",
            i
        );
        assert_eq!(
            original_message.sender_id, deserialized.sender_id,
            "Message {} sender_id should match",
            i
        );
        assert_eq!(
            original_message.timestamp, deserialized.timestamp,
        ...
        match (&original_message.message_type, &deserialized.message_type) {
            (
                ...
                assert_eq!(
                    t1, t2,
                    "TextChat target_user_id should match for message {}",
                    i
                );
            ...
            (
                ...
                assert_eq!(
                    t1, t2,
                    "WebRTCSignaling target_user_id should match for message {}",
                    i
                );
                assert_eq!(
                    d1, d2,
                    "WebRTCSignaling data should match for message {}",
                    i
                );
            }
            (
                ...
                assert_eq!(
                    t1, t2,
                    "GenericMessage target_user_id should match for message {}",
                    i
                );
                assert_eq!(
                    c1, c2,
                    "GenericMessage content should match for message {}",
                    i
                );
            }
