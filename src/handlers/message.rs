...
impl MessageHandler {
    ...
    async fn send_to_user(&self, target_id: &ClientId, message: Message) -> Result<(), ConnectionError> {
                ...
                client.sender.send(message)
                    .map_err(|_| {
                        ...
                        ConnectionError::DeliveryFailed(
                            format!("Failed to deliver message to user {}", target_id)
                        )
                    })?;
                debug!(
                    target_id = %target_id,
    ...
    async fn broadcast_message(&self, sender_id: &ClientId, message: Message) -> Result<(), ConnectionError> {
        let clients = self.clients.read().await;
        ...
        if !failed_deliveries.is_empty() {
            return Err(ConnectionError::DeliveryFailed(
                format!("Failed to deliver broadcast message to {} clients: {:?}", 
...
mod tests {
    ...
    async fn test_direct_message_success() {
        ...
        let result = handler.handle_text_chat(
            "client_1".to_string(),
    ...
    async fn test_broadcast_with_single_client() {
        ...
        clients.write().await.insert("solo_client".to_string(), client);
    ...
    async fn test_get_broadcast_recipient_count() {
        ...
        let count = handler.get_broadcast_recipient_count(&"client_1".to_string()).await;
        assert_eq!(count, 2); // client_2 and client_3

        let count = handler.get_broadcast_recipient_count(&"non_existent".to_string()).await;
    ...
    async fn test_send_to_user_direct() {
        ...
        let result = handler.send_to_user(&"client_1".to_string(), message.clone()).await;
        assert!(result.is_ok());
    ...
    async fn test_broadcast_message_direct() {
        ...
        let result = handler.broadcast_message(&"client_1".to_string(), message.clone()).await;
        assert!(result.is_ok());
    ...
    async fn test_concurrent_message_handling() {
        ...
        for i in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                handler_clone.handle_text_chat(
                    "client_1".to_string(),
                    Some("client_2".to_string()),
                    format!("Message {}", i),
                ).await
    ...
    async fn test_generic_message_success() {
        ...
        let result = handler.handle_generic_message(
            "client_1".to_string(),
            "client_2".to_string(),
    ...
    async fn test_generic_message_preserve_content() {
        ...
        let complex_content = r#"{"command": "custom_action", "params": {"key": "value", "array": [1, 2, 3]}}"#;
    ...
    async fn test_validate_generic_content() {
        ...
        assert!(!MessageHandler::validate_generic_content(&"a".repeat(10001))); // Too large (>10KB)
    }
    ...
    async fn test_generic_message_different_content_types() {
        ...
        for (i, content) in test_contents.iter().enumerate() {
            ...
            let received_message = client_2_receiver.recv().await.unwrap();
            match received_message.message_type {
                MessageType::GenericMessage { content: received_content, .. } => {
    ...
    async fn test_concurrent_generic_messages() {
        ...
        for i in 0..10 {
            let handler_clone = Arc::clone(&handler);
            let handle = tokio::spawn(async move {
                handler_clone.handle_generic_message(
                    "client_1".to_string(),
                    "client_2".to_string(),
                    format!("command_{}", i),
                ).await
