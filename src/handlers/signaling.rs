...
impl SignalingHandler {
    ...
    pub async fn handle_webrtc_signaling(
        ...
        let target_client_id = match self
            .connection_manager
            ...
            .await
        {
            ...
            None => {
                if self
                    .connection_manager
                    .client_exists(&target_username)
                    .await
                {
                    info!(
                        sender_id = %sender_id,
                        target_identifier = %target_username,
                        "Falling back to client ID match for signaling target"
                    );
                    ...
                    return Err(ConnectionError::ClientNotFound(target_username.clone()));
                }
            }
        };
        ...
        let sender_username = self
            .connection_manager
            .find_username_by_client_id(&sender_id)
            .await;
        ...
        let result = self
            .forward_signaling_message(&target_client_id, message)
            .await;
    ...
    pub async fn handle_ice_candidate(
        ...
        self.handle_webrtc_signaling(sender_id, target_username, candidate_data)
            .await
    }
    ...
    pub async fn handle_sdp_message(
        ...
        self.handle_webrtc_signaling(sender_id, target_username, sdp_data)
            .await
    }
}
...
mod tests {
    ...
    async fn test_ice_candidate_invalid_data() {
        ...
        let result = handler
            .handle_ice_candidate("client_1".to_string(), "client_2".to_string(), invalid_data)
            .await;
    ...
    async fn test_sdp_invalid_data() {
        ...
        let result = handler
            .handle_sdp_message("client_1".to_string(), "client_2".to_string(), invalid_sdp)
            .await;
    ...
    async fn test_signaling_message_with_sender_identification() {
        ...
        let result = handler
            .handle_webrtc_signaling(sender_id.clone(), "client_2".to_string(), signaling_data)
            .await;
